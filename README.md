...
- **API 地址可配置**：留空默认使用 `https://www.recode.cat`，不可用时依次切换到 `https://www.reclaude.ai`、`https://reclaude.ai`。
...
| `reclaude.apiBase` | `""` | API 根地址；留空时默认 `https://www.recode.cat`，不可用时依次切到 `https://www.reclaude.ai`、`https://reclaude.ai`；填写后仅使用该地址 |
...
```bash
code --install-extension dist/reclaude-monitor-0.0.4.vsix --force
```
